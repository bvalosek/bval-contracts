...
library TokenID {
  ...
  function tokenMintTimestamp(uint256 tokenId) internal pure returns (uint) {
    uint16 daystamp = uint16(tokenId >> 26*8);
    return uint(daystamp) * 60 * 60 * 24;
  }

  function tokenYieldMultiplier(uint256 tokenId) internal pure returns (uint16) {
    return uint16(tokenId >> 24*8);
  }

  function tokenBurnMultiplier(uint256 tokenId) internal pure returns (uint16) {
    return uint16(tokenId >> 22*8);
  }
